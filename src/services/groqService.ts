...
export const verifyTranslationQualityWithGroq = async (params: {
  ...
  const { apiKey, verifierModel, pageNumber, translatedText, signal, legalContext = true, sourceLanguage = "Tedesco", customPrompt } = params;
  if (!apiKey) throw new Error("API Key Groq mancante");
  ...
  // Groq text-only: verifica solo testo, senza immagini
  const messages = [
    { role: 'system', content: systemPrompt },
    {
      role: 'user',
      content: `TRADUZIONE DA VERIFICARE (Pagina ${pageNumber}):\n"""\n${translatedText.slice(0, 6000)}\n"""`
    }
  ];
